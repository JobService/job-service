...
job_service_api_endpoint = '/job-service/v1/partitions/{partition_id}/jobs/{job_id}'
...
def parse_args():
    ...
    required_named.add_argument('-P', '--partition', dest='partitionId', help='partition identifier', required=True, metavar='partitionId')
...
def validate_args(args):
    ...
    if args.partitionId == "":
        log('The partition identifier is empty. Exiting.')
...
def main():
    ...
    uri = (args.jobWebServiceURL +
        job_service_api_endpoint.format(partition_id=partition_id, job_id=job_id))
