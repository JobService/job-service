...
/**
 * Monitors RabbitMQ publisher confirmations, message returns, and channel shutdowns to determine if message publication failures are
 ...
 * </ul>
 * <p>
 * This class is automatically registered as a listener on the provided channel. Based on its analysis, it either leaves the job to be
 * retried (transient failure) or marks it as permanently failed and removes it from the job_task_data table (non-transient failure).
 */
public final class RabbitMqAsyncListener
{
    ...
    {
        /**
         * A temporary failure that may be resolved by retrying later. Examples include network issues, full queues, or a broker under
         * heavy load.
         */
        TRANSIENT,
        /**
         * A permanent failure that will not be resolved by retrying. Examples include unroutable messages, invalid permissions, or
         * protocol errors.
         */
        NON_TRANSIENT
    }
        ...
        final String targetQueue)
    {
        ...
        final RabbitMqAsyncListener listener = new RabbitMqAsyncListener(partitionId, jobId, targetQueue);

        // Register this listener to receive all relevant events.
        channel.addConfirmListener(listener.new ConfirmListenerImpl());
        channel.addReturnListener(listener.new ReturnListenerImpl());
        channel.addShutdownListener(listener.new ShutdownListenerImpl());
    }

    private RabbitMqAsyncListener(final String partitionId, final String jobId, final String targetQueue)
    {
    ...
    private final class ConfirmListenerImpl implements ConfirmListener
    {
        /**
         * Handles a successful message acknowledgment from the broker. This indicates that the message has been accepted and safely
         * handled by the broker. The job is considered successfully processed and is removed from the system.
         *
        ...
        public void handleAck(final long deliveryTag, final boolean multiple) throws IOException
        {
            ...
            // The channel is only used for a single message so we can ignore the multiple parameter.
        ...
        /**
         * Handles a message rejection (NACK) from the broker. NACKs are typically caused by transient issues, such as a queue reaching
         * its capacity, so they are treated as retryable failures.
         *
    ...
    private final class ReturnListenerImpl implements ReturnListener
    {
        /**
         * Handles messages that were published as 'mandatory' but could not be routed to any queue. This is considered a permanent,
    ...
    {
        /**
         * Handles an unexpected channel or connection shutdown. The shutdown cause is analyzed to determine if the failure is transient
         * or permanent. Application-initiated shutdowns are ignored.
         *
    ...
    /**
     * Performs cleanup operations for a successfully completed job. This includes releasing cached resources and removing the job from
     * the database.
     */
     ...
     *
     * <p>
     * For transient failures, it simply logs the event and cleans up resources, allowing the job to be picked up again for a retry.
     *
     * <p>
     * For non-transient failures, it marks the job as permanently failed in the database and then removes it from the task data table to
