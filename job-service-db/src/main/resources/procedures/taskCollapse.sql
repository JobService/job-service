...
BEGIN
    ...
    SELECT array(
        SELECT id FROM unnest( tasks ) AS id
    ...
    WHILE (modified='t') LOOP
            ...
            WHILE (cardinality(tasks)!=0) LOOP
                    ...
                    -- if there's no more task, then we return the final_array
            ...
            ELSE
                -- sorting tasks
                select array(SELECT id FROM unnest( final_array ) as id order BY substring(id,regex_body), substring(id, regex_last_nb)::INTEGER desc ) into tasks;
