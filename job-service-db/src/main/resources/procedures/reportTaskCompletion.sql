...
BEGIN
    ...
    IF v_final_task_id IS NOT NULL THEN
        ...
        v_sub_tasks_to_expect = CAST(substring(
            v_final_task_id, v_sub_task_count_starting_position, length(v_final_task_id) - v_sub_task_count_starting_position) AS INT);

        -- Base percentage completed on the number of sub tasks expected
        EXECUTE format($FORMAT_STR$
            SELECT round(((SELECT COUNT(task_id) FROM %1$I WHERE status = 'Completed') * 100)::numeric / (%2$s), 2)
                AS completed_percentage
        $FORMAT_STR$, in_task_table_name, v_sub_tasks_to_expect)
        INTO v_percentage_completed;
    ELSE
        -- Final sub-task has not yet arrived. Adjust percentage to allow for one more entry to arrive.
        EXECUTE format($FORMAT_STR$
            SELECT round(((SELECT COUNT(task_id) FROM %1$I WHERE status = 'Completed') * 100)::numeric /
                          (SELECT COUNT(*) + 1 FROM %1$I), 2)
                AS completed_percentage
    ...
    ELSE
        ...
        EXECUTE format($FORMAT_STR$
            ...
            SELECT %3$L, now() AT TIME ZONE 'UTC', 'Completed'::job_status, %2$L, NULL, %4$L
            WHERE NOT EXISTS (SELECT * FROM upsert)
        $FORMAT_STR$, v_parent_table_name, v_percentage_completed, v_task_id, v_is_final_task);
