...
BEGIN
	...
	-- Store task data if any of the prerequisite job identifiers are not yet complete.
	IF EXISTS (SELECT 1 FROM public.job WHERE job_id = ANY(in_prerequisite_job_ids) AND status <> 'Completed')
	THEN  
