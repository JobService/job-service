...
public class JobServiceIT {
    ...
    @Test
    public void testCreateJobSuspendedPartition() throws ApiException {
        //create a job for a suspended partition
        String jobId = UUID.randomUUID().toString();
        String jobCorrelationId = "1";
        final NewJob newJob = makeJob(jobId, "testCreateJobSuspendedPartition");
        final String partitionId = "tenant-acme-com";
        jobsApi.createOrUpdateJob(partitionId, jobId, newJob, jobCorrelationId);

        //retrieve job using web method
        Job retrievedJob = jobsApi.getJob(partitionId, jobId, jobCorrelationId);
    ...
    @Test
    public void testCreateJobWithPreReqSuspendedPartition() throws ApiException {
        //create a job with pre reqs for a suspended partition
        String jobId = UUID.randomUUID().toString();
        String jobCorrelationId = "1";
        final NewJob newJob = makeJob(jobId, "testCreateJobWithPreReqSuspendedPartition");
    ...
    @Test
    public void testCreateJobWithDelaySuspendedPartition() throws ApiException {
        //create a job with delay for a suspended partition
        String jobId = UUID.randomUUID().toString();
        String jobCorrelationId = "1";
        final NewJob newJob = makeJob(jobId, "testCreateJobWithDelaySuspendedPartition");
    ...
    @Test
    public void testCreateJobWithDelayWithPreReqSuspendedPartition() throws ApiException {
        //create a job with delay and pre reqs for a suspended partition
        String jobId = UUID.randomUUID().toString();
        String jobCorrelationId = "1";
        final NewJob newJob = makeJob(jobId, "testCreateJobWithDelayWithPreReqSuspendedPartition");
        newJob.setPrerequisiteJobIds(Collections.singletonList(UUID.randomUUID().toString()));
        newJob.setDelay(600); // 600s
